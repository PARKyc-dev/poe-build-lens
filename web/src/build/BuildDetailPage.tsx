...
export function BuildDetailPage({ result, analysis, onNewInspection }: {
  ...
  return <main className="build-detail" aria-label="빌드 상세">
    ...
    {tooltip && <DetailTooltipCard tooltip={tooltip} style={{ position: 'fixed', left: tooltipLeft, top: tooltipTop, maxHeight: `calc(100vh - ${tooltipTop + 12}px)` }} ariaLabel={`${tooltip.title} ${['공격', '방어', '버프', '저주/징표', '패시브', '마스터리', '전직'].includes(tooltip.label) ? '상세 정보' : '장비 정보'}`} />}
  </main>
}
