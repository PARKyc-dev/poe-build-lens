...
export function SkillSections({ result, analysis, onShow, onHide }: { result: BrowserInspectResult; analysis: BuildAnalysisResult; onShow: ShowDetailTooltip; onHide: () => void }) {
  ...
  return <><SkillSection title="공격 기재" emptyMessage="분석할 공격 기재가 없습니다." entries={attacks} mechanics={analysis.offence} onShow={(event, entry) => onShow(event, { id: `attack:${entry.name}`, label: '공격', title: entry.name, details: entry.details })} onHide={onHide} /><SkillSection title="방어 기재" emptyMessage="분석할 방어 기재가 없습니다." entries={defence} mechanics={analysis.defence} onShow={(event, entry) => onShow(event, { id: `defence:${entry.name}`, label: '방어', title: entry.name, details: entry.details })} onHide={onHide} /><SkillSection title="버프 기재" emptyMessage="표시할 버프 기재가 없습니다." entries={buffsOnly} mechanics={analysis.buffs} onShow={(event, entry) => onShow(event, { id: `buff:${entry.name}`, label: '버프', title: entry.name, details: entry.details })} onHide={onHide} /><SkillSection title="이동기 기재" emptyMessage="표시할 이동기가 없습니다." entries={mobility} mechanics={[]} onShow={(event, entry) => onShow(event, { id: `mobility:${entry.name}`, label: '이동기', title: entry.name, details: entry.details })} onHide={onHide} /></>
}
